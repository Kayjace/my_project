...
def main():
    try:
        ...
        if database_names:
            ...
            # 기본 설정 파일을 사용
            logging.info("Using default database settings from connection.yaml.")
            try:
                engine, metadata, inspector, tables = setup_database(connection_config)
            except Exception as e:
                logging.error(f"Skipping default database due to setup error: {e}", exc_info=True)
                return
